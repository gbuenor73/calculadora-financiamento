...
export const AmortizationSidebar: React.FC<AmortizationSidebarProps> = ({
    ...
    return (
        <Card
            ...
            headerAction={
                <button onClick={onAdd} className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors">
                    ...
                    <div className="space-y-3">
                        {amortizations.map((amort) => (
                            <div key={amort.id} className="p-4 bg-slate-50 border border-slate-200/60 rounded-xl space-y-3 relative group transition-all hover:bg-white hover:shadow-sm">
                                <button onClick={() => onRemove(amort.id)} className="absolute top-3 right-3 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Valor do Aporte</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className={`w-full pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm font-black text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all ${amort.amount >= loanAmount && loanAmount > 0 ? 'border-amber-400' : ''}`}
                                        />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    <div className={`${amort.frequency === 'monthly' ? 'col-span-2' : 'col-span-1'} space-y-1`}>
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Frequência</label>
                                        <select
                                    ...
                                    {amort.frequency !== 'monthly' && (
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{amort.frequency === 'yearly' ? 'Mês' : 'Mês nº'}</label>
                                            <input
                                                type="number"
                                                min="1"
